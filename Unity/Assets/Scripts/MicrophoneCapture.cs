...
public class MicrophoneCapture : MonoBehaviour
{
    ...
    **/
    const string PROJECT_ID = "nuicovidagent-jomvll";
    const string email = "harishnuicovid@nuicovidagent-jomvll.iam.gserviceaccount.com";
    /* Make sure it is in the Assets Folder so that this is only the name */
    string certPath = "nuicovidagent.p12";

    // This needs to be changed to access you script
    public chatBehavior myScript;
    ...
    void Start()
    {
        ...
        this.myScript = GameObject.FindObjectOfType(typeof(chatBehavior)) as chatBehavior;
    ...
    void OnGUI()
    {
        ...
        if (micConnected)
        {
            // GUI.Button(new Rect(Screen.width / 4 - 100, Screen.height / 4 - 25, 200, 50
            ...
            if (!Microphone.IsRecording(null))
            {
                ...
                if (GUI.Button(new Rect(Screen.width / 4 - 100, Screen.height / 4 - 25, 200, 50), "Record"))
                {
                    ...
                    //handle dialogflow
                     ...
                     StartCoroutine(myScript.startAck());
                }
            }
            else //Recording is in progress
            {
                ...
                if (GUI.Button(new Rect(Screen.width / 4 - 100, Screen.height / 4 - 25, 200, 50), "Stop and Play!"))
                {
                    ...
                    //send out request
                    StopListening();
                    StartCoroutine(myScript.stopAck());
                    // StartCoroutine(myScript.startTyping());
                }
    ...
    IEnumerator StartVoiceRequest(String url, String AccessToken, object parameter)
    {
        ...
        if (samples != null)
        {
            ...
            else
            {
                ...
                //StartCoroutine(myScript.stopTyping());
