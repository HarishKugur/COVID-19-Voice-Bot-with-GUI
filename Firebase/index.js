...
exports.dialogflowFirebaseFulfillment = functions.https.onRequest((request, response) => {
    ...
    function getCurrentDateStats(types, countryCode, countryName, date, agent) {
        const dateProcessed = resolveEndDate(date); 
        var statFlag = true;
        ...
        agent.add("Single Date: " + dateProcessed);
        ...
        return getJSON('https://coronavirus-tracker-api.ruizlab.org/v2/locations?source=jhu&country_code=' + countryCode + '&timelines=true').then((result) => {
            ...
            if (statFlag) {
                ...
                agent.add(`Sorry, the requested stat is not avalaible for that date.`);
    ...
    async function worldwideApiRequest(types) {
        var worldwideCount = [];
        try {
            let result = await getJSON('https://coronavirus-tracker-api.ruizlab.org/v2/latest?source=jhu');
            worldwideCount.push(result.latest);
            displayStats(worldwideCount, types);
    ...
    async function worldwideLatestStats(agent) {
        // agent.add("Welcome to Worldwide latest stats");

        const types = agent.parameters.type;
        let promiseResolved = await worldwideApiRequest(types);
    }
    ...
    function displayCountyInfo(response, counties, types, state) {
        // agent.add("Calling displayCountyInfo");
    ...
    function displayStatesInfo(response, states, types) {
        // agent.add("Calling displayStatesInfo");
    ...
    // var countriesList = ["AU", "CA", "CN", "DK", "FR", "NL", "GB"];
    var stats = {};
